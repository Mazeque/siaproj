...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document.querySelectorAll(".btn-remove").forEach((remove) => {
    remove.addEventListener("click", function () {
      // KEY
      const removeId = parseInt(remove.getAttribute("removeid"));

      $.ajax({
        url: "php-addons/removeitemcart.php",
        data: {
          cartid: removeId,
        },
        method: "POST",
        success: function (resp) {
          if (resp) {
            const itemcont = document.querySelector(".box-for-" + removeId);
            const parentelement = itemcont.parentNode;

            parentelement.style.transition = "all .6s";
            parentelement.style.height = parentelement.offsetHeight + "px";

            const currentHeight = itemcont.offsetHeight;

            const parentheight = parentelement.offsetHeight;

            itemcont.style.height = currentHeight + "px";
            ...
            setTimeout(function () {
              const getprice = parseFloat(
                document.querySelector('span[totalpriceof="' + removeId + '"]')
                  .textContent
              );

              const cbox = document.querySelector(
                'input[value="' + removeId + '"]'
              );
              ...
              if (cbox.checked) {
                ...
                gettotalprice.innerHTML = (
                  parseFloat(gettotalprice.textContent) - parseFloat(getprice)
                ).toFixed(2);
              ...
              document.getElementById("select-all-button").removeEventListener("click", selectall);
              document.getElementById("select-all-button").addEventListener("click", selectall);

              parentelement.removeChild(itemcont);
              parentelement.style.height = parentheight - currentHeight + "px";
            }, 500);
          }
        },
      });
