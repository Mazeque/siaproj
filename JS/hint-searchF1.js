...
const DEFAULTS = {
  ...
  class Autocomplete {
    ...
    createItem(lookup, item) {
      ...
      return ce(
        `<button type="button" style = "font-size: 11.3px; color: #707070; letter-spacing: 0.5px;"class="dropdown-item py-2" id="${item.id}" data-label="${item.label}" data-value="${item.value}">${label}</button>`
      );
    }
    ...
    createItems() {
        ...
        for (let i = 0; i < keys.length; i++) {
          ...
          const item = {
            ...
            id: this.options.id ? entry[this.options.id] : entry.id
          };
