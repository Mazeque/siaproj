...
function proceedToLink(checked) {
    ...
    setTimeout(function() {
        document.getElementById('containerpay').style.transform = "translateX(-150%)";
    },200);

    setTimeout(function() {
        $('#payment-box').load('php-addons/loadpaymentlink.php?method=' + checked, function() {
         $.getScript('JS/cardscript.js');
        });

        setTimeout(function() {
            document.getElementById('container-pm').style.transform = "translateX(0%)";
...
const pmmodal = document.getElementById('mod-content');

function handleOutsideClick(event) {
    if (!pmmodal.contains(event.target)) {
      section = 1;
