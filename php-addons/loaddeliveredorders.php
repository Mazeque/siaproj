...
AS total_price, p.order_information FROM payment p JOIN cart c ON p.cart_id = c.cart_id WHERE p.user_id = ? AND p.order_status = ? GROUP BY p.order_id;";

$orderstatus = "Delivered";

$stmt = mysqli_prepare($conn, $query);
$stmt->bind_param("is", $_SESSION['userid'], $orderstatus);
$stmt->execute();
                            ...
                            <div class="col-lg-3 col-12 center-all mt-4 mt-lg-0">
                                <div class="col-12 d-flex justify-content-lg-end justify-content-center row">
                                    ...
                                    <div class="col-12  d-flex justify-content-lg-end justify-content-center pt-3 px-0">
                                        ...
                                        <?php else: ?>
                                            <span class="leave-a-review" data-bs-toggle="modal" data-bs-target="#review_modal" orderid="<?php echo $row['order_id'] ?>" productid = "<?php echo $orderrow['product_id']?>"><i
                                                    class="fa-solid fa-pen-to-square"></i> Leave a Review</span>
                                        <?php endif; ?>
                                    </div>
                                </div>
                            </div>
