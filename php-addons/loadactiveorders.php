...
AS total_price, p.order_information FROM payment p JOIN cart c ON p.cart_id = c.cart_id WHERE p.user_id = ? AND p.order_status = ? OR p.order_status = ? GROUP BY p.order_id;";

$orderstatus1 = "Processing";
$orderstatus2 = "On-delivery";

$stmt = mysqli_prepare($conn, $query);
$stmt->bind_param("iss", $_SESSION['userid'], $orderstatus1, $orderstatus2);
$stmt->execute();
