...
AS total_price, p.order_information FROM payment p JOIN cart c ON p.cart_id = c.cart_id WHERE p.order_status = ? OR p.order_status = 'On-delivery' GROUP BY p.order_id;";
        ...
        <div class="col-12 order-cont p-3 mt-4">
            ...
            <?php if ($row['order_status'] == 'Processing') : ?>
                    <div class="col-12 mt-3">
                        <button class = "btn btn-outline-success w-100 process-order" id = "process-order" orderid = "<?php echo $row['order_id']?>">Process Order</button>
                    </div>
                    <?php endif; ?>
        </div>
